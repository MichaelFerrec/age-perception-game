...
const AuthSection = ({ onBack, onSuccess }: AuthSectionProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: isLogin ? "Bienvenue de retour !" : "Votre compte a été créé avec succès"
    });
        ...
        <Card className="border-0 shadow-2xl bg-white/90 backdrop-blur-sm">
          <CardHeader className="text-center pb-8">
            ...
            <CardDescription className="text-gray-600 text-lg">
              {isLogin ? 'Bon retour parmi nous !' : 'Rejoins la communauté AgeGuess'}
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-6">
            <form onSubmit={handleSubmit} className="space-y-4">
              {!isLogin && (
                <div className="space-y-2">
                  <Label htmlFor="name" className="text-gray-700 font-medium">Nom ou pseudo</Label>
            ...
            <Button 
              variant="outline" 
              className="w-full py-6 text-lg border-gray-200 hover:bg-gray-50"
              onClick={() => {
                toast({
                  title: "Bientôt disponible !",
                  description: "La connexion Google sera ajoutée prochainement"
                });
              }}
            >
